    ...
    fetch('./partner.json')
        ...
        .then(data => {
            for (p of data.partners) {
                ...
                //
                fetchAndSetBackgroundImage(p.IMG, p['IMG-HOVER'], div)

                div.setAttribute('url', `${p.url}`)
                if (p['Name'] == 'DOMINOS' || p['Name'] == 'NATIXIS' ){
                    div.style.position = "relative"
                    div.classList.add('rodrigo-special');
...
class Event {
    constructor(node_main) {
        ...
        this.know_more.innerText = "SOBRE"
        ...
        this.know_more.addEventListener("click", () => {
            ...
            document.getElementById("modal-title").innerText = `Oque sera abordado? 🎯`
    ...
    setAvatar(content) {
        fetchAndSetBackgroundImage(content, null, this.avatar)
...
function fetchAndSetBackgroundImage(imageUrl, imageUrlHover, target) {
    if (imageUrlHover != null) {
        target.addEventListener('mouseover', (event) => {
            target.style.backgroundImage = `url(${imageUrlHover})`;
        });
